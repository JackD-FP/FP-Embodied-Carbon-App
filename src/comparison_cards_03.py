...
card03 = html.Div([
    ...
    html.Div(id='card3_output'),
    html.Div(id="card3_contents")
])


@callback(Output('card3_output', 'children'),
              Input('card3_upload_data', 'contents'),
              State('card3_upload_data', 'filename'),
              State('card3_upload_data', 'last_modified'))
def update_output(list_of_contents, list_of_names, list_of_dates):
    if list_of_contents is not None:
        children = [
            uploader.parse_contents(c, n, d, "card3_temp_store", "name_3") for c, n, d in
            zip(list_of_contents, list_of_names, list_of_dates)]
        return children


@callback(
Output('card3_contents', 'children'),
Input('card03_store', 'modified_timestamp'), 
State('card03_store', 'data'),
)
def card3_content_update(mts, data,):
    if data is None or mts is None:
        ...
        return html.Div([
            html.H3("Comparison 3", className="display-5 my-3"),
...
# ----------- Green book comparison for CARD 2 ----------            
            ...
            dmc.Accordion([
                dmc.AccordionItem(
                    material_table.table_gen(
                        ...
                        html.Div(id="gb_card3_timber_val"),
                    ),
            ...
            dbc.Row([
                dbc.Col([
                    html.Div(id="gb_card3_total", className="text-center"),
                    html.P([html.Span(["kgCO",html.Sup(2),html.Sub('e')], className="fs-4"), " Total EC"], className="text-center")
                ]),
                dbc.Col([
                    html.Div(id="gb_card3_gfa", className="text-center"),
...
# ----------- EPiC comparison for CARD 2 ----------    
            ...
            dmc.Accordion([
                dmc.AccordionItem(
                    material_table.table_gen(
                        ...
                        html.Div(id="epic_card3_timber_val"),
                    ),
            ...
            dbc.Row([
                dbc.Col([
                    html.Div(id="epic_card3_total", className="text-center"),
                    html.P([html.Span(["kgCO",html.Sup(2),html.Sub('e')], className="fs-4"), " Total EC"], className="text-center")
                ]),
                dbc.Col([
                    html.Div(id="epic_card3_gfa", className="text-center"),
                    html.P([html.Span(["kgCO",html.Sup(2),html.Sub('e'),'/m',html.Sup(2)], className="fs-4"), " EC per m", html.Sup(2)], className="text-center")
                ]),
            ], className="my-5"),
...
# ----------- ICE comparison for CARD 2 ----------    
            ...
            dmc.Accordion([
                dmc.AccordionItem(
                    material_table.table_gen(
                        ...
                        html.Div(id="ice_card3_timber_val"),
                    ),
            ...
            dbc.Row([
                dbc.Col([
                    html.Div(id="ice_card3_total", className="text-center"),
                    html.P([html.Span(["kgCO",html.Sup(2),html.Sub('e')], className="fs-4"), " Total EC"], className="text-center")
                ]),
                dbc.Col([
                    html.Div(id="ice_card3_gfa", className="text-center"),
                    html.P([html.Span(["kgCO",html.Sup(2),html.Sub('e'),'/m',html.Sup(2)], className="fs-4"), " EC per m", html.Sup(2)], className="text-center")
                ]),
            ], className="my-4"),
            html.Div(id="card3_ice_pie"),
...
def card3_total_gfa_update(val, conc_val, steel_val, timber_val, data):
    if val is None:
        ...
        return html.H3("{:,}".format(total)), html.H3("{:,}".format(np.around(total_per_m2, 2))), epic_concrete, epic_steel, epic_timber, pie_child
...
def card3_total_gfa_update(val, conc_val, steel_val, timber_val, data):
    if val is None:
        ...
        return html.H3("{:,}".format(np.around(total,2))), html.H3("{:,}".format(np.around(total_per_m2, 2))), ice_concrete, ice_steel, ice_timber, pie_child
